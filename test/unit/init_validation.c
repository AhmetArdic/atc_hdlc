...
static atc_hdlc_plat_ops_t make_valid_plat(void) {
    atc_hdlc_plat_ops_t p;
    memset(&p, 0, sizeof(p));
...
static atc_hdlc_txwin_t make_valid_tw(void) {
    atc_hdlc_txwin_t tw;
    tw.slots = s_tx_slots;
...
static atc_hdlc_rxbuf_t make_valid_rx(void) {
    atc_hdlc_rxbuf_t rx;
...
void test_init_null_params(void) {
    ...
    atc_hdlc_plat_ops_t plat = make_valid_plat();
    atc_hdlc_txwin_t tw = make_valid_tw();
    atc_hdlc_rxbuf_t rx = make_valid_rx();
    atc_hdlc_params_t p = {.config = &cfg, .platform = &plat, .tx_window = &tw, .rx_buf = &rx};

    /* NULL ctx */
    if (atc_hdlc_init(NULL, p) != ATC_HDLC_ERR_BAD_PARAM)
        test_fail("Init NULL", "NULL ctx should return INVALID_PARAM");

    /* NULL config */
    p.config = NULL;
    if (atc_hdlc_init(&ctx, p) != ATC_HDLC_ERR_BAD_PARAM)
    ...
    /* NULL on_send */
    atc_hdlc_plat_ops_t bad_plat = plat;
...
void test_init_unsupported_mode(void) {
    ...
    cfg.mode = (atc_hdlc_mode_t)1;
    if (atc_hdlc_init(&ctx, p) != ATC_HDLC_ERR_BAD_MODE)
        test_fail("Init Mode", "non-ABM mode should return UNSUPPORTED_MODE");

    cfg.mode = (atc_hdlc_mode_t)2;
    if (atc_hdlc_init(&ctx, p) != ATC_HDLC_ERR_BAD_MODE)
        test_fail("Init Mode", "non-ABM mode should return UNSUPPORTED_MODE");
...
void test_init_inconsistent_rx_buffer(void) {
    ...
    atc_hdlc_rxbuf_t rx_small = {.buffer = small_buf, .capacity = 1024};
    p.rx_buf = &rx_small;
    if (atc_hdlc_init(&ctx, p) != ATC_HDLC_ERR_BAD_BUF)
...
void test_init_inconsistent_tx_window(void) {
    ...
    p.tx_window = &bad_tw;
    if (atc_hdlc_init(&ctx, p) != ATC_HDLC_ERR_BAD_BUF)
        test_fail("Init TX Window", "slot_capacity < max_info_size should return INCONSISTENT_BUFFER");
...
static const test_entry_t s_tests[] = {{"test_init_null_params", test_init_null_params},
