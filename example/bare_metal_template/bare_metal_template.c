 ...
 */
static void t1_timer_isr(void) { t1_expired_flag = true; }
...
static void t2_timer_isr(void) { t2_expired_flag = true; }
...
static const atc_hdlc_platform_t platform = {
    ...
    .on_data  = on_data,
    .on_event = on_event,
    .t1_start = t1_start, .t1_stop = t1_stop,
    .t2_start = t2_start, .t2_stop = t2_stop,
};
...
int main(void)
{
    ...
    if (t1_expired_flag) { t1_expired_flag = false; atc_hdlc_t1_expired(&ctx); }
    if (t2_expired_flag) { t2_expired_flag = false; atc_hdlc_t2_expired(&ctx); }
