...
{
    switch (e) {
    case ATC_HDLC_EVENT_LINK_SETUP_REQUEST:  return "LINK_SETUP_REQUEST";
    case ATC_HDLC_EVENT_CONNECT_ACCEPTED:    return "CONNECT_ACCEPTED";
    case ATC_HDLC_EVENT_INCOMING_CONNECT:    return "INCOMING_CONNECT";
    case ATC_HDLC_EVENT_RESET:               return "RESET";
    case ATC_HDLC_EVENT_DISCONNECT_REQUEST:  return "DISCONNECT_REQUEST";
    case ATC_HDLC_EVENT_DISCONNECT_COMPLETE: return "DISCONNECT_COMPLETE";
    case ATC_HDLC_EVENT_PEER_DISCONNECT:     return "PEER_DISCONNECT";
    case ATC_HDLC_EVENT_PEER_REJECT:         return "PEER_REJECT";
    case ATC_HDLC_EVENT_PROTOCOL_ERROR:      return "PROTOCOL_ERROR";
    case ATC_HDLC_EVENT_LINK_FAILURE:        return "LINK_FAILURE";
    case ATC_HDLC_EVENT_REMOTE_BUSY_ON:      return "REMOTE_BUSY_ON";
    case ATC_HDLC_EVENT_REMOTE_BUSY_OFF:     return "REMOTE_BUSY_OFF";
    case ATC_HDLC_EVENT_WINDOW_OPEN:         return "WINDOW_OPEN";
    default:                                 return "?";
    }
}

static void on_event_a(atc_hdlc_event_t e, void *u) { (void)u; printf("[A] %s\n", event_str(e)); }
static void on_event_b(atc_hdlc_event_t e, void *u) { (void)u; printf("[B] %s\n", event_str(e)); }
...
static void t_start(atc_hdlc_u32 ms, void *u) { (void)ms; (void)u; }
static void t_stop(void *u) { (void)u; }
...
static const atc_hdlc_platform_t plat_a = {
    .on_send  = send_cb,
    .on_event = on_event_a,
    .t1_start = t_start, .t1_stop = t_stop,
    .t2_start = t_start, .t2_stop = t_stop,
    .user_ctx = &wire_ab,
};
...
int main(void)
{
    atc_hdlc_init(&ctx_a, (atc_hdlc_params_t){
        .config = &cfg_a, .platform = &plat_a,
        .tx_window = &tx_win_a, .rx_buf = &rx_buf_a,
    });
    atc_hdlc_init(&ctx_b, (atc_hdlc_params_t){
        .config = &cfg_b, .platform = &plat_b,
